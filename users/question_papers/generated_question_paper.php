        ...
        <div class="card-body">
            ...
            <style>
                ...
                <?php if ($single > 0): ?>
                    <div class="page-break">
                        ...
                        <?php 
                        $i = 1;
                        $questions = $conn->query("SELECT * FROM `question_list` WHERE question_paper_id = '{$id}' AND `type` = 1 ORDER BY `id` ASC LIMIT {$single}");
                ...
                <div class="page-break">
                    ...
                    <?php 
                    $i = 1;
                    $questions = $conn->query("SELECT * FROM `question_list` WHERE question_paper_id = '{$id}' AND `type` = 2 ORDER BY `id` ASC LIMIT {$multiple}");
                    while ($row = $questions->fetch_assoc()):
                    ?>
            ...
            <?php endif; ?>
                ...
                <div class="page-break">
                    ...
                    <?php 
                    $i = 1;
                    $questions = $conn->query("SELECT * FROM `question_list` WHERE question_paper_id = '{$id}' AND `type` = 3 ORDER BY `id` ASC LIMIT {$text}");
                    while ($row = $questions->fetch_assoc()):
                    ?>
                    <div class="question-item mb-3">
                        <div class="d-flex w-100 mb-1">
...
<script>
    $(function(){
        $('#print').click(function(){
            var courseName = "<?= isset($title) ? strtoupper($title) : 'Generated Question Paper' ?>";
            var headClone = $('head').clone();
            var outprintClone = $('#outprint').clone();
            var printContainer = $('<div>');

            headClone.find('title').text(courseName);
