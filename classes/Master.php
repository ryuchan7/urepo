...
Class Master extends DBConnection {
	...
	function save_course(){
		...
		$check = $this->conn->query("SELECT * FROM `programme_list` where `name` = '{$name}' and user_id = '{$uid}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Programme Name already exists.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `programme_list` set {$data} ";
		}else{
			$sql = "UPDATE `programme_list` set {$data} where id = '{$id}' ";
		}
			$save = $this->conn->query($sql);
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "New Programme successfully saved.";
			else
				$resp['msg'] = " Programme successfully updated.";
	...
	function delete_course(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `programme_list` set `delete_flag` = 1 where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," programme successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_class(){
		...
		$check = $this->conn->query("SELECT * FROM `course_list` where `name` = '{$name}' and user_id = '{$uid}' and `course_id` = '{$course_id}' and user_id = '{$this->settings->userdata('id')}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
